...
FOR EACH gi_data_banks WHERE gi_data_banks.bank-code = pbank-code
    ...
    IF gi_data_banks.refnum1 BEGINS "CV" THEN DO:
        ASSIGN prefnum-temp = REPLACE(gi_data_banks.refnum1," ","").
        ASSIGN prefnum-temp = REPLACE(gi_data_banks.refnum1,"CV","CDV").
    END.
    ...
    FOR EACH gi_acc_det WHERE gi_acc_det.acc-led-name = gi_data_banks.bank-des
                          AND (
                              (gi_data_bank.amt-db > 0  AND gi_acc_det.debit = gi_data_bank.amt-db)
                               OR 
                              (gi_data_bank.amt-cr > 0 AND gi_acc_det.credit = gi_data_bank.amt-cr)
                              )
