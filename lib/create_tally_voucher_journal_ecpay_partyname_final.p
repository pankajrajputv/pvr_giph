DEF SHARED VAR pecpay-sum-cons LIKE gi_data_ecpay_sum.ecpay-sum-cons.
...
FOR EACH gi_data_ecpay_sum WHERE gi_data_ecpay_sum.ecpay-sum-cons = pecpay-sum-cons NO-LOCK
    BREAK BY gi_data_ecpay_sum.ecpay-sum-cons:
    
    /*
    FIND FIRST gi_data_ecpay WHERE gi_data_ecpay. gi_data_ecpay_sum NO-LOCK NO-ERROR.
    IF AVAILABLE gi_data_ecpay THEN DO:
        ASSIGN pecpaysname = gi_data_ecpay.ecpaysname.
    END.
    ELSE DO:
        ASSIGN pecpaysname = "".
    END.
    */
    ...
    IF FIRST-OF(gi_data_ecpay_sum.ecpay-sum-cons) THEN DO:
        ASSIGN 
               pacc-ledger-name = replace(gi_data_ecpay_sum.party-name,"&","&amp;")
               pacc-ledger-name-party = replace(gi_data_ecpay_sum.party-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.
        
        ASSIGN 
               pacc-ledger-name = replace(gi_data_ecpay_sum.alias-name,"&","&amp;")
               pacc-ledger-name-alias = replace(gi_data_ecpay_sum.alias-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.


        RUN c:\pvr\giph\lib\Calc_Vou_Date_From_Fn.p(INPUT gi_data_ecpay_sum.YearMonthFN,OUTPUT pvoudate).

        ASSIGN 
            pvouchertype         = "Journal-Ecpay"
            pvouchernumber       = /*"CDV36462"*/ "JE-" + STRING(gi_data_ecpay_sum.ecpay-sum-cons,"999999")
            /*pvouchernumber       = /*"CDV36462"*/  STRING(gi_data_ecpay_sum.ecpay-sum-cons)*/
            pvoucherdate         = /*10/31/19*/ pvoudate
            peffectivedate       = /*10/31/19*/ pvoudate
            pchequedate          = /*10/31/19*/ pvoudate
            pchequenumber        = /*STRING(gi_data_ecpay_sum.cheque-num)*/ ""
            ...
            pnarration           = replace(gi_data_ecpay_sum.party-name,"&","&amp;")
            .
    ...
    END. /* IF FIRST-OF(gi_data_ecpay_sum.ecpay-sum-cons) */

    /*************** DEBIT *****************/
    DEF VAR pcontrol-agent-narr AS CHAR FORMAT "X(200)".
    ASSIGN pcontrol-agent-narr = STRING(gi_data_ecpay_sum.TotalAmount,"-zz,zzz,zz9.99") + " + " + STRING(gi_data_ecpay_sum.agentcomm,"-zz,zzz,zz9.99").


    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = Db-ControlAgent * -1 pledgernarration = "Gross Amount / " +  STRING((gi_data_ecpay_sum.CollectFromAgent - gi_data_ecpay_sum.AgentComm),"-zz,zzz,zz9.99").
    RUN debit_entry.

    ASSIGN pledgername-debit1 = "Agent Commission - EcPay" pledgeramount-debit1 = gi_data_ecpay_sum.Db-agentcomm * -1 pledgernarration = "Agent Commission / " + STRING(gi_data_ecpay_sum.agentcomm,"-zz,zzz,zz9.99").
    RUN debit_entry.


    IF gi_data_EcPay_sum.diffGP > 0 THEN DO:
        ...
        ASSIGN pledgername-debit1 = "EcPay Sales" pledgeramount-debit1 = gi_data_EcPay_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN debit_entry.
        ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_EcPay_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_EcPay_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN credit_entry.
    END.      
    ...
    /*ASSIGN pledgername-debit1 = "Input Vat @ 12%" pledgeramount-debit1 = gi_data_ecpay_sum.inputvat * -1 pledgernarration = "Input Vat".*/
    ASSIGN pledgername-debit1 = "VAT Input Tax" pledgeramount-debit1 = gi_data_ecpay_sum.inputvat * -1 pledgernarration = "Input Vat".
    RUN debit_entry.
      */

    /*
    IF gi_data_ecpay_sum.total-db-cr-diff < 0 THEN DO:
        ASSIGN pledgername-debit1 = "Income - Miscellaneous" pledgeramount-debit1 = gi_data_ecpay_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN debit_entry.
    END.
      */
    /*************** CREDIT *****************/

    ASSIGN pledgername-credit1 = "EcPay Sales" pledgeramount-credit1 = gi_data_ecpay_sum.Cr-EcPaySales pledgernarration = "". 
    /*ASSIGN pledgername-credit1 = "EcPay" pledgeramount-credit1 = gi_data_ecpay_sum.comm-income1 pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_ecpay_sum.TotalAmount - gi_data_ecpay_sum.PayableToOperator,"-zz,zzz,zz9.99"). */
    RUN credit_entry.

    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_ecpay_sum.Cr-EcPayPurchases pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_ecpay_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.

    IF gi_data_EcPay_sum.diffGP < 0 THEN DO:
        ASSIGN pledgername-credit1 = "EcPay Sales" pledgeramount-credit1 = gi_data_EcPay_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN credit_entry.
        
        ASSIGN pledgername-debit1 = pacc-ledger-name-alias pledgeramount-debit1 = gi_data_EcPay_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_EcPay_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN debit_entry.
    ...
    /*ASSIGN pledgername-credit1 = "Payable to Operate - ecpay" pledgeramount-credit1 = gi_data_ecpay_sum.payable1 pledgernarration = "Payable. / " /*+ STRING(gi_data_ecpay_sum.mtagentcomm,"-zz,zzz,zz9.99") */.*/

    /*
    ASSIGN pledgername-credit1 = "Agent Commission - ecpay" pledgeramount-credit1 = gi_data_ecpay_sum.agentcomm  pledgernarration = "Agent Commission / " + STRING(gi_data_ecpay_sum.agentcomm1,"-zz,zzz,zz9.99").
    RUN credit_entry.
      */

    /*
    /*ASSIGN pledgername-credit1 = "Output Vat @ 12%" pledgeramount-credit1 = gi_data_ecpay_sum.outputvat pledgernarration = "Commission Income - ecpay / x 0.12 " + STRING(gi_data_ecpay_sum.comm-income1, "-zz,zzz,zz9.99").*/
    ASSIGN pledgername-credit1 = "Output Tax Payable" pledgeramount-credit1 = gi_data_ecpay_sum.outputvat pledgernarration = "Commission Income - ecpay / x 0.12 " + STRING(gi_data_ecpay_sum.comm-income1, "-zz,zzz,zz9.99").
    RUN credit_entry.
      */
    /*
    IF gi_data_ecpay_sum.total-db-cr-diff > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Income - Miscellaneous" pledgeramount-credit1 = gi_data_ecpay_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN credit_entry.
    END.
     */
END. /* for each gi_data_ecpay_sum */
...
FOR EACH gi_data_ecpay_sum WHERE gi_data_ecpay_sum.ecpay-sum-cons = pecpay-sum-cons:
    RUN c:\pvr\giph\lib\get_voucher_id.p(INPUT poutput-filename, OUTPUT pLastVchId, OUTPUT gi_data_ecpay_sum.tally-response).
    /*
    ASSIGN gi_data_ecpay_sum.LastVchId = pLastVchId.
    */
END.
