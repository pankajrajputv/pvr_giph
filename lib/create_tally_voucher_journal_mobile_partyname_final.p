DEF SHARED VAR pmobile-sum-cons LIKE gi_data_mobile_sum.mobile-sum-cons.
...
FOR EACH gi_data_mobile_sum WHERE gi_data_mobile_sum.mobile-sum-cons = pmobile-sum-cons NO-LOCK
    BREAK BY gi_data_mobile_sum.mobile-sum-cons:
    
    /*
    FIND FIRST gi_data_mobile WHERE gi_data_mobile. gi_data_mobile_sum NO-LOCK NO-ERROR.
    IF AVAILABLE gi_data_mobile THEN DO:
        ASSIGN pmobilesname = gi_data_mobile.mobilesname.
    END.
    ELSE DO:
        ASSIGN pmobilesname = "".
    END.
    */
    ...
    IF FIRST-OF(gi_data_mobile_sum.mobile-sum-cons) THEN DO:
        ASSIGN 
               pacc-ledger-name = replace(gi_data_mobile_sum.party-name,"&","&amp;")
               pacc-ledger-name-party = replace(gi_data_mobile_sum.party-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.
        
        ASSIGN 
               pacc-ledger-name = replace(gi_data_mobile_sum.alias-name,"&","&amp;")
               pacc-ledger-name-alias = replace(gi_data_mobile_sum.alias-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.

        RUN c:\pvr\giph\lib\Calc_Vou_Date_From_Fn.p(INPUT gi_data_mobile_sum.YearMonth,OUTPUT pvoudate).

        ASSIGN 
            pvouchertype         = "Journal-Mobile"
            pvouchernumber       = /*"CDV36462"*/ "JM-" + STRING(gi_data_mobile_sum.mobile-sum-cons,"999999")
            /*pvouchernumber       = /*"CDV36462"*/  STRING(gi_data_mobile_sum.mobile-sum-cons)*/
            pvoucherdate         = /*10/31/19*/ pvoudate
            peffectivedate       = /*10/31/19*/ pvoudate
            pchequedate          = /*10/31/19*/ pvoudate
            pchequenumber        = /*STRING(gi_data_mobile_sum.cheque-num)*/ ""
            ...
            pnarration           = replace(gi_data_mobile_sum.party-name,"&","&amp;")
            .
    ...
    END. /* IF FIRST-OF(gi_data_mobile_sum.mobile-sum-cons) */

    /*************** DEBIT *****************/
    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = gi_data_mobile_sum.db-ControlAgent * -1 pledgernarration = "Collectable / " +  STRING((gi_data_mobile_sum.collectable),"-zz,zzz,zz9.99").
    RUN debit_entry.

    /*
    IF gi_data_mobile_sum.total-db-cr-diff < 0 THEN DO:
        ASSIGN pledgername-debit1 = "Mobile Sales" pledgeramount-debit1 = gi_data_mobile_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN debit_entry.
    END.
      */
    
    IF gi_data_mobile_sum.diffGP < 0 THEN DO:
        /*
        MESSAGE gi_data_mobile_sum.diffGP
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
            */
        ASSIGN pledgername-debit1 = "Mobile Sales" pledgeramount-debit1 = gi_data_mobile_sum.diffGP   pledgernarration = "Diff GP / " + STRING("").
        RUN debit_entry.
    END.
    
    
    /*
    ASSIGN pledgername-debit1 = "Agent Commission - mobile" pledgeramount-debit1 = gi_data_mobile_sum.agentcomm1 * -1 pledgernarration = "Agent Commission / " + STRING(gi_data_mobile_sum.agentcomm1,"-zz,zzz,zz9.99").
    RUN debit_entry.


    ASSIGN pledgername-debit1 = "Input Vat @ 12%" pledgeramount-debit1 = gi_data_mobile_sum.inputvat * -1 pledgernarration = "Input Vat".
    RUN debit_entry.
    */
    ...
    /*ASSIGN pledgername-credit1 = "Payable to Operate - mobile" pledgeramount-credit1 = gi_data_mobile_sum.payable1 pledgernarration = "Payable. / " /*+ STRING(gi_data_mobile_sum.mtagentcomm,"-zz,zzz,zz9.99") */.*/
    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_mobile_sum.cr-MobilePurchases pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_mobile_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.


    ASSIGN pledgername-credit1 = "Mobile Sales" pledgeramount-credit1 = (gi_data_mobile_sum.cr-MobileSales) pledgernarration = "Supplier Comm / " + STRING(gi_data_mobile_sum.SupplierComm,"-zz,zzz,zz9.99"). 
    /*ASSIGN pledgername-credit1 = "Mobile" pledgeramount-credit1 = gi_data_mobile_sum.comm-income1 pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_mobile_sum.GrossAmount - gi_data_mobile_sum.Payable,"-zz,zzz,zz9.99"). */
    RUN credit_entry.

    /*ASSIGN pledgername-credit1 = "Output Vat @ 12%" pledgeramount-credit1 = gi_data_mobile_sum.outputvat pledgernarration = "Commission Income - mobile / x 0.12 " + STRING(gi_data_mobile_sum.comm-income1, "-zz,zzz,zz9.99").*/
    /*
    ASSIGN pledgername-credit1 = "Output Tax Payable" pledgeramount-credit1 = gi_data_mobile_sum.outputvat pledgernarration = "Commission Income - mobile / x 0.12 " + STRING(gi_data_mobile_sum.comm-income1, "-zz,zzz,zz9.99").
    RUN credit_entry.
    */

    /*
    IF gi_data_mobile_sum.total-db-cr-diff > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Income - Miscellaneous" pledgeramount-credit1 = gi_data_mobile_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN credit_entry.
    END.
      */
    
    IF gi_data_mobile_sum.diffGP > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Mobile Sales" pledgeramount-credit1 = gi_data_mobile_sum.diffGP  pledgernarration = "Diff GP / " + STRING(gi_data_mobile_sum.Profit,"-zz,zzz,zz9.99") + " " + STRING(gi_data_mobile_sum.cr-MobileSales,"-zz,zzz,zz9.99").
        RUN credit_entry.
    END.
    
END. /* for each gi_data_mobile_sum */
...
FOR EACH gi_data_mobile_sum WHERE gi_data_mobile_sum.mobile-sum-cons = pmobile-sum-cons:
    RUN c:\pvr\giph\lib\get_voucher_id.p(INPUT poutput-filename, OUTPUT pLastVchId, OUTPUT gi_data_mobile_sum.tally-response).
    /*
    ASSIGN gi_data_mobile_sum.LastVchId = pLastVchId.
    */
END.
