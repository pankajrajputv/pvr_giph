DEF SHARED VAR pferry-sum-cons LIKE gi_data_ferry_sum.ferry-sum-cons.
...
FOR EACH gi_data_ferry_sum WHERE gi_data_ferry_sum.ferry-sum-cons = pferry-sum-cons 
    BREAK BY gi_data_ferry_sum.ferry-sum-cons:
    
    /*
    FIND FIRST gi_data_ferry WHERE gi_data_ferry. gi_data_ferry_sum NO-LOCK NO-ERROR.
    IF AVAILABLE gi_data_ferry THEN DO:
        ASSIGN pferrysname = gi_data_ferry.ferrysname.
    END.
    ELSE DO:
        ASSIGN pferrysname = "".
    END.
    */
    ...
    IF FIRST-OF(gi_data_ferry_sum.ferry-sum-cons) THEN DO:
        ASSIGN 
               pacc-ledger-name = replace(gi_data_ferry_sum.party-name,"&","&amp;")
               pacc-ledger-name-party = replace(gi_data_ferry_sum.party-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.
        
        IF gi_data_ferry_sum.alias-name = "" THEN DO:
            ASSIGN gi_data_ferry_sum.alias-name = "Ferry Purchases".
        END.
        ASSIGN 
               pacc-ledger-name = replace(gi_data_ferry_sum.alias-name,"&","&amp;")
               pacc-ledger-name-alias = replace(gi_data_ferry_sum.alias-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.

        RUN c:\pvr\giph\lib\Calc_Vou_Date_From_Fn.p(INPUT gi_data_ferry_sum.YearMonthFN,OUTPUT pvoudate).

        ASSIGN 
            pvouchertype         = "Journal-Ferry"
            pvouchernumber       = /*"CDV36462"*/ "JF-" + STRING(gi_data_ferry_sum.ferry-sum-cons,"999999")
            /*pvouchernumber       = /*"CDV36462"*/  STRING(gi_data_ferry_sum.ferry-sum-cons)*/
            pvoucherdate         = /*10/31/19*/ pvoudate
            peffectivedate       = /*10/31/19*/ pvoudate
            pchequedate          = /*10/31/19*/ pvoudate
            pchequenumber        = /*STRING(gi_data_ferry_sum.cheque-num)*/ ""
            ...
            pnarration           = replace(gi_data_ferry_sum.party-name,"&","&amp;")
            .
    ...
    END. /* IF FIRST-OF(gi_data_ferry_sum.ferry-sum-cons) */

    /*************** DEBIT *****************/
    DEF VAR pcontrol-agent-narr AS CHAR FORMAT "x(200)".

    ASSIGN pcontrol-agent-narr = 
        STRING(gi_data_ferry_sum.vatableamount,"-zz,zzz,zz9.99") + " + " +
        STRING(gi_data_ferry_sum.vat,"-zz,zzz,zz9.99") +  " + " +
        STRING(gi_data_ferry_sum.vatexempt,"-zz,zzz,zz9.99") +  " + " +
        STRING(gi_data_ferry_sum.zerovat,"-zz,zzz,zz9.99") + " + " +
        STRING(gi_data_ferry_sum.transactionfee,"-zz,zzz,zz9.99") +  " + " +
        STRING(gi_data_ferry_sum.penalty,"-zz,zzz,zz9.99") +  " + " +
        STRING(gi_data_ferry_sum.handlingfee,"-zz,zzz,zz9.99") + 
        STRING(gi_data_ferry_sum.agentcomm,"-zz,zzz,zz9.99") .

    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = gi_data_ferry_sum.db-ControlAgent * -1 pledgernarration = STRING(pcontrol-agent-narr) .
    RUN debit_entry.

    ASSIGN pledgername-debit1 = "Agent Commission - Ferry" pledgeramount-debit1 = gi_data_ferry_sum.db-AgentComm * -1 pledgernarration = "Agent Commission / " + STRING(gi_data_ferry_sum.agentcomm,"-zz,zzz,zz9.99").
    RUN debit_entry.
              
    IF gi_data_ferry_sum.diffGP > 0 THEN DO:
        ...
        ASSIGN pledgername-debit1 = "Ferry Sales" pledgeramount-debit1 = gi_data_ferry_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN debit_entry.
        ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_ferry_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_ferry_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN credit_entry.
    END.      


    /*
    /*ASSIGN pledgername-debit1 = "Input Vat @ 12%" pledgeramount-debit1 = gi_data_ferry_sum.inputvat * -1 pledgernarration = "Input Vat".*/
    ASSIGN pledgername-debit1 = "VAT Input Tax" pledgeramount-debit1 = gi_data_ferry_sum.inputvat * -1 pledgernarration = "Input Vat".
    RUN debit_entry.
          */
                  
    IF gi_data_ferry_sum.total-db-cr-diff < 0 THEN DO:
       /* ASSIGN pledgername-debit1 = "Income - Miscellaneous" pledgeramount-debit1 = gi_data_ferry_sum.total-db-cr-diff * -1 pledgernarration = "Diff Db Cr / " + STRING("").*/
        ASSIGN pledgername-debit1 = "Ferry Sales" pledgeramount-debit1 = gi_data_ferry_sum.total-db-cr-diff pledgernarration = "Diff Db Cr / " + STRING("").
        RUN debit_entry.
    END.            
      
    /*************** CREDIT *****************/
    DEF VAR pferry-sales-narr AS CHAR FORMAT "x(200)".
    ASSIGN pferry-sales-narr = STRING(gi_data_ferry_sum.penalty,"-zz,zzz,zz9.99") +  " + " +
        STRING(gi_data_ferry_sum.handlingfee,"-zz,zzz,zz9.99").

    ASSIGN pledgername-credit1 = "Ferry Sales" pledgeramount-credit1 = gi_data_ferry_sum.cr-ferrySales pledgernarration =  pferry-sales-narr. 
    /*ASSIGN pledgername-credit1 = "Ferry" pledgeramount-credit1 = gi_data_ferry_sum.comm-income1 pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_ferry_sum.GrossAmount - gi_data_ferry_sum.Payable,"-zz,zzz,zz9.99"). */
    RUN credit_entry.


    /*ASSIGN pledgername-credit1 = "Payable to Operate - ferry" pledgeramount-credit1 = gi_data_ferry_sum.payable1 pledgernarration = "Payable. / " /*+ STRING(gi_data_ferry_sum.mtagentcomm,"-zz,zzz,zz9.99") */.*/
    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_ferry_sum.cr-ferryPurchases pledgernarration = "Payable. / " + pacc-ledger-name-party + STRING(gi_data_ferry_sum.payable,"-zz,zzz,zz9.99") .
    RUN credit_entry.


    /*
    /*ASSIGN pledgername-credit1 = "Output Vat @ 12%" pledgeramount-credit1 = gi_data_ferry_sum.outputvat pledgernarration = "Commission Income - ferry / x 0.12 " + STRING(gi_data_ferry_sum.comm-income1, "-zz,zzz,zz9.99").*/
    ASSIGN pledgername-credit1 = "Output Tax Payable" pledgeramount-credit1 = gi_data_ferry_sum.outputvat pledgernarration = "Commission Income - ferry / x 0.12 " + STRING(gi_data_ferry_sum.comm-income1, "-zz,zzz,zz9.99").
    RUN credit_entry.
      */

                      /*
    IF gi_data_ferry_sum.total-db-cr-diff > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Ferry Sales" pledgeramount-credit1 = gi_data_ferry_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN credit_entry.
    END.               */
    
    IF gi_data_ferry_sum.diffGP < 0 THEN DO:
        ASSIGN pledgername-credit1 = "Ferry Sales" pledgeramount-credit1 = gi_data_ferry_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN credit_entry.
        
        ASSIGN pledgername-debit1 = pacc-ledger-name-alias pledgeramount-debit1 = gi_data_ferry_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_ferry_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN debit_entry.
        
    END.
                        

END. /* for each gi_data_ferry_sum */
...
FOR EACH gi_data_ferry_sum WHERE gi_data_ferry_sum.ferry-sum-cons = pferry-sum-cons:
    RUN c:\pvr\giph\lib\get_voucher_id.p(INPUT poutput-filename, OUTPUT pLastVchId, OUTPUT gi_data_ferry_sum.tally-response).
    /*
    ASSIGN gi_data_ferry_sum.LastVchId = pLastVchId.
    */
END.
