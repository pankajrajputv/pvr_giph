DEF SHARED VAR pbus-sum-cons LIKE gi_data_bus_sum.bus-sum-cons.
...
FOR EACH gi_data_bus_sum WHERE gi_data_bus_sum.bus-sum-cons = pbus-sum-cons /*NO-LOCK*/
    BREAK BY gi_data_bus_sum.bus-sum-cons:
    
    /*
    FIND FIRST gi_data_bus WHERE gi_data_bus. gi_data_bus_sum NO-LOCK NO-ERROR.
    IF AVAILABLE gi_data_bus THEN DO:
        ASSIGN pbussname = gi_data_bus.bussname.
    END.
    ELSE DO:
        ASSIGN pbussname = "".
    END.
    */
    ...
    IF FIRST-OF(gi_data_bus_sum.bus-sum-cons) THEN DO:
        ASSIGN 
               pacc-ledger-name = replace(gi_data_bus_sum.party-name,"&","&amp;")
               pacc-ledger-name-party = replace(gi_data_bus_sum.party-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.
        
        ASSIGN 
               pacc-ledger-name = replace(gi_data_bus_sum.alias-name,"&","&amp;")
               pacc-ledger-name-alias = replace(gi_data_bus_sum.alias-name,"&","&amp;")
               pacc-ledger-parent-name = "Accounts Payable". 
        RUN CreateLedger.


        RUN c:\pvr\giph\lib\Calc_Vou_Date_From_Fn.p(INPUT gi_data_bus_sum.YearMonthFN,OUTPUT pvoudate).

        ASSIGN 
            pvouchertype         = "Journal-Bus"
            pvouchernumber       = /*"CDV36462"*/ "JB-" + STRING(gi_data_bus_sum.bus-sum-cons,"999999")
            /*pvouchernumber       = /*"CDV36462"*/  STRING(gi_data_bus_sum.bus-sum-cons)*/
            pvoucherdate         = /*10/31/19*/ pvoudate
            peffectivedate       = /*10/31/19*/ pvoudate
            pchequedate          = /*10/31/19*/ pvoudate
            pchequenumber        = /*STRING(gi_data_bus_sum.cheque-num)*/ ""
            ...
            pnarration           = replace(gi_data_bus_sum.party-name,"&","&amp;")
            .
    ...
    END. /* IF FIRST-OF(gi_data_bus_sum.bus-sum-cons) */
    ...
    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = Db-ControlAgent * -1 pledgernarration = "Gross Amount / " +  STRING((gi_data_Bus_sum.CollectFromAgent - gi_data_Bus_sum.AgentComm),"-zz,zzz,zz9.99").
    RUN debit_entry.

    ASSIGN pledgername-debit1 = "Agent Commission - Bus" pledgeramount-debit1 = gi_data_Bus_sum.Db-agentcomm * -1 pledgernarration = "Agent Commission / " + STRING(gi_data_Bus_sum.agentcomm,"-zz,zzz,zz9.99").
    RUN debit_entry.


    IF gi_data_Bus_sum.diffGP > 0 THEN DO:
        ...
        ASSIGN pledgername-debit1 = "Bus Sales" pledgeramount-debit1 = gi_data_Bus_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN debit_entry.
        ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_Bus_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_Bus_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN credit_entry.
    END.      
    ...
    ASSIGN pledgername-credit1 = "Bus Sales" pledgeramount-credit1 = gi_data_Bus_sum.Cr-BusSales pledgernarration = "". 
    /*ASSIGN pledgername-credit1 = "Bus" pledgeramount-credit1 = gi_data_Bus_sum.comm-income1 pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_Bus_sum.TotalAmount - gi_data_Bus_sum.PayableToOperator,"-zz,zzz,zz9.99"). */
    RUN credit_entry.

    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_Bus_sum.Cr-BusPurchases pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_Bus_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.

    IF gi_data_Bus_sum.diffGP < 0 THEN DO:
        ASSIGN pledgername-credit1 = "Bus Sales" pledgeramount-credit1 = gi_data_Bus_sum.diffGP * -1   pledgernarration = "Diff GP / " + STRING("").
        RUN credit_entry.
        
        ASSIGN pledgername-debit1 = pacc-ledger-name-alias pledgeramount-debit1 = gi_data_Bus_sum.diffGP pledgernarration = "Adjustment. / " + pacc-ledger-name-party + STRING(gi_data_Bus_sum.diffGP,"-zz,zzz,zz9.99") .
        RUN debit_entry.
...
FOR EACH gi_data_bus_sum WHERE gi_data_bus_sum.bus-sum-cons = pbus-sum-cons:
    RUN c:\pvr\giph\lib\get_voucher_id.p(INPUT poutput-filename, OUTPUT pLastVchId, OUTPUT gi_data_bus_sum.tally-response).
    /*
    ASSIGN gi_data_bus_sum.LastVchId = pLastVchId.
    */
END.
...
/*
    ...
    /*
    ASSIGN gi_data_bus_sum.GrossAmount1 = gi_data_bus_sum.Amount + gi_data_bus_sum.ServiceCharge.
      */

    /*************** DEBIT *****************/
    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = gi_data_bus_sum.collectfromagent * -1 pledgernarration = "Gross Amount / " +  STRING((gi_data_bus_sum.CollectFromAgent - gi_data_bus_sum.AgentComm),"-zz,zzz,zz9.99").
    RUN debit_entry.

    /*
    ASSIGN pledgername-debit1 = "Agent Commission - Bus" pledgeramount-debit1 = gi_data_bus_sum.agentcomm1 * -1 pledgernarration = "Agent Commission / " + STRING(gi_data_bus_sum.agentcomm1,"-zz,zzz,zz9.99").
    RUN debit_entry.


    /*ASSIGN pledgername-debit1 = "Input Vat @ 12%" pledgeramount-debit1 = gi_data_bus_sum.inputvat * -1 pledgernarration = "Input Vat".*/
    ASSIGN pledgername-debit1 = "VAT Input Tax" pledgeramount-debit1 = gi_data_bus_sum.inputvat * -1 pledgernarration = "Input Vat".
    RUN debit_entry.
      */
    /*
    IF gi_data_bus_sum.total-db-cr-diff < 0 THEN DO:
        ASSIGN pledgername-debit1 = "Income - Miscellaneous" pledgeramount-debit1 = gi_data_bus_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN debit_entry.
    END.
      */
    ...
    ASSIGN pledgername-credit1 = "Bus Sales" pledgeramount-credit1 = gi_data_bus_sum.servicecharge pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_bus_sum.GrossAmount - gi_data_bus_sum.Payable,"-zz,zzz,zz9.99"). 
    /*ASSIGN pledgername-credit1 = "Bus" pledgeramount-credit1 = gi_data_bus_sum.comm-income1 pledgernarration = "Gross Amt - Payable / " + STRING(gi_data_bus_sum.GrossAmount - gi_data_bus_sum.Payable,"-zz,zzz,zz9.99"). */
    RUN credit_entry.


    /*ASSIGN pledgername-credit1 = "Payable to Operate - Bus" pledgeramount-credit1 = gi_data_bus_sum.payable1 pledgernarration = "Payable. / " /*+ STRING(gi_data_bus_sum.mtagentcomm,"-zz,zzz,zz9.99") */.*/
    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_bus_sum.payable pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_bus_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.



    /*
    /*ASSIGN pledgername-credit1 = "Output Vat @ 12%" pledgeramount-credit1 = gi_data_bus_sum.outputvat pledgernarration = "Commission Income - bus / x 0.12 " + STRING(gi_data_bus_sum.comm-income1, "-zz,zzz,zz9.99").*/
    ASSIGN pledgername-credit1 = "Output Tax Payable" pledgeramount-credit1 = gi_data_bus_sum.outputvat pledgernarration = "Commission Income - bus / x 0.12 " + STRING(gi_data_bus_sum.comm-income1, "-zz,zzz,zz9.99").
    RUN credit_entry.
      */
    /*
    IF gi_data_bus_sum.total-db-cr-diff > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Income - Miscellaneous" pledgeramount-credit1 = gi_data_bus_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN credit_entry.
    END.
     */
