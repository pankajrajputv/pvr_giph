...
FOR EACH gi_data_packages_sum WHERE gi_data_packages_sum.packages-sum-cons = ppackages-sum-cons NO-LOCK
    ...
    */
    ...
    /*************** DEBIT *****************/
    ASSIGN pledgername-debit1 = "Control Agent" pledgeramount-debit1 = gi_data_packages_sum.Db-ControlAgent * -1 pledgernarration = "Coll. from Agent / " +  STRING((gi_data_packages_sum.CollectFromAgent),"-zz,zzz,zz9.99").
    RUN debit_entry.
    ...
    IF gi_data_packages_sum.total-db-cr-diff < 0 THEN DO:
        ...
        ASSIGN pledgername-debit1 = "Packages Sales" pledgeramount-debit1 = gi_data_packages_sum.total-db-cr-diff pledgernarration = "Diff Db Cr / " + STRING("").
        RUN debit_entry.
    END.
    ...
    ASSIGN pledgername-credit1 = "Packages Sales" pledgeramount-credit1 = gi_data_packages_sum.Cr-PackagesSales pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_packages_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.

    ASSIGN pledgername-credit1 = pacc-ledger-name-alias pledgeramount-credit1 = gi_data_packages_sum.Cr-PackagesPurchases pledgernarration = "Payable. / " + pacc-ledger-name-party /*+ STRING(gi_data_packages_sum.mtagentcomm,"-zz,zzz,zz9.99") */.
    RUN credit_entry.
    ...
    IF gi_data_packages_sum.total-db-cr-diff > 0 THEN DO:
        ASSIGN pledgername-credit1 = "Packages Sales" pledgeramount-credit1 = gi_data_packages_sum.total-db-cr-diff  pledgernarration = "Diff Db Cr / " + STRING("").
        RUN credit_entry.
    END.
